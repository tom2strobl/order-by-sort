...
const entityArray = [
  ...
  { id: 3, rating: null, author: "Frank", starred: true, date: "2021-01-01" },
...
test("throw on missing field", () => {
  const orderByArray = [{ field: "reader", value: "asc_nulls_last" }];
  expect(() => orderBySort(entityArray, orderByArray)).toThrow(
    `Object in array is missing field ${orderByArray[0].field}`
  );
});
...
test("throw on invalid field type", () => {
  ...
  const orderByArray = [{ field: "rating", value: "asc_nulls_last" }];
  expect(() => orderBySort(malformedEntityArray, orderByArray)).toThrow(
    `Field ${orderByArray[0].field} is of invalid type, only string, number and date are allowed`
  );
});

test("throw on invalid order operator", () => {
  const orderByArray = [{ field: "rating", value: "asc_null_last" }];
  expect(() => orderBySort(entityArray, orderByArray)).toThrow(
    `${orderByArray[0].value} is an invalid OrderOperator`
  );
});
