...
var determineResult = function (a, b, orderEntry) {
    ...
    if (!Object.keys(a).includes(orderEntry.field) ||
        !Object.keys(b).includes(orderEntry.field)) {
        throw new Error("Object in array is missing field " + orderEntry.field);
    }
    if (!isValidField(a[orderEntry.field]) ||
        !isValidField(b[orderEntry.field])) {
        throw new Error("Field " + orderEntry.field + " is of invalid type, only string, number and date are allowed");
    }
    // normalize fields for comparison. note: We type cast, since we know that the fields are valid from above
    var fieldA = normalizeField(a[orderEntry.field]);
    var fieldB = normalizeField(b[orderEntry.field]);
