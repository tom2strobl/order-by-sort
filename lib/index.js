...
const compare = (a, b, direction, nullPosition) => {
  ...
  if (a === b) {
    return 0;
      ...
      return nullPosition === 'first' ? 1 : -1;
        ...
        const cleanedA = isNumber(a) ? a * 1 : a.toLowerCase(); // restore numbers

        const cleanedB = isNumber(b) ? b * 1 : b.toLowerCase(); // otherwise, if we're ascending, lowest sorts first
...
const determineResult = (a, b, orderEntry) => {
  ...
  const fieldA = a[orderEntry.field];
  const fieldB = b[orderEntry.field];
...
function orderBySort(arr, orderArray) {
  ...
  const sort = (a, b, remainingOrderArray = orderArray) => {
    // calculate the compare value of the first(=next) item to compare with
    const compareValue = determineResult(a, b, remainingOrderArray[0]); // See if the next key needs to be considered

    const checkNextKey = compareValue === 0 && remainingOrderArray.length !== 1; // Return compare value (potential recursion)
